...
class SubscriptionService {
    ...
    async refreshCookieFollowings() {
        ...
        try {
            ...
            if (needAll) {
                logger.info('\[SubscriptionService\] Fetching ALL followings...');
                ...
                if (res.status === 'success' && res.data) {
                    ...
                    logger.warn(`[SubscriptionService] Failed to fetch ALL followings: ${res.message}`);
                }
            }

            // 3. Fetch specific groups
            for (const groupName of neededGroups) {
                logger.info(`[SubscriptionService] Fetching followings for group: ${groupName}...`);
                ...
                if (res.status === 'success' && res.data) {
                    ...
                    logger.warn(`[SubscriptionService] Failed to fetch followings for group ${groupName}: ${res.message}`);
                }
            }
